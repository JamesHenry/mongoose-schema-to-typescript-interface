...
export function extendRefTypes(
  ...
  refPaths.forEach(refPath => {
    ...
    const targetInterfaceStartRegexp = new RegExp(
     ...
     */
    const refInterfaceStartRegexp = new RegExp(
      `interface ${INTERFACE_PREFIX}${refValue} {`,
    );
    ...
    const matchingReferencedInterfaceName: string = stripInterface(
      foundInterface[0],
    );
    ...
    const updatedLines = outputLines.map((line, index) => {
      if (
        index > startIndexOfTargetInterface &&
        index < endIndexOfTargetInterface
      ) {
        const targetFieldDefinition = line.match(refPropertyRegexp);
        if (targetFieldDefinition) {
          return line.replace(
            'string;',
            `string | ${matchingReferencedInterfaceName};`,
          );
        }
      }
